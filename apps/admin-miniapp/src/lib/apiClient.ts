...
export const api = {
  ...
  put: <T>(path: string, schema: z.ZodType<T>, body?: unknown) => request(path, schema, { method: 'PUT', body }),
  del: <T>(path: string, schema: z.ZodType<T>) => request(path, schema, { method: 'DELETE' })
}
