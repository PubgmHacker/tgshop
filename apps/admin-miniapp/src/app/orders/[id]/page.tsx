...
import { OrderDetailResponseSchema, RefundResponseSchema } from '@/types/api'

function InfoRow({ label, value }: { label: string; value: React.ReactNode }): JSX.Element {
  return (
    <div className="flex items-center justify-between gap-3 px-3 py-2.5">
      <span className="text-[13px] text-muted">{label}</span>
      <span className="tnum min-w-0 truncate text-right text-[13px] font-semibold text-ink">{value}</span>
    </div>
  )
}
    ...
    <main className="page-enter flex flex-col gap-4 pt-2">
      ...
      >
        {({ order, payments, ledger }) => {
          const buyer = order.user.username ? `@${order.user.username}` : order.user.firstName ?? order.user.tgId
            ...
            <>
                ...
                <div className="glass relative rounded-[22px] p-4">
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 truncate text-lg font-bold tracking-[-0.02em] text-ink">{order.productTitle}</p>
                      <p className="truncate text-sm text-muted">{order.planTitle}</p>
                    </div>
                    <div className="flex shrink-0 flex-col items-end gap-1.5">
                      <span className="tnum text-lg font-bold text-ink">{formatCents(order.amountCents)}</span>
              ...
              <section className="mx-4 flex flex-col gap-2.5">
                <div className="panel flex flex-col divide-y divide-line rounded-card">
                  ...
                  <InfoRow label={t('order.paid')} value={order.paidAt ? formatDate(order.paidAt, locale) : '—'} />
                  <InfoRow
                  ...
                  <div className="panel flex flex-col divide-y divide-line rounded-card">
                    {payments.map((payment) => (
                      <div key={payment.id} className="flex items-center justify-between gap-3 p-3">
                        <div className="min-w-0">
                          <p className="text-sm font-semibold text-ink">{t(`provider.${payment.provider}`)}</p>
                          <p className="truncate text-xs text-muted">
                            {payment.amount}
                            {payment.asset ? ` ${payment.asset}` : ''} · {formatDate(payment.createdAt, locale)}
                          </p>
                          {payment.txHash ? (
              ...
              <BottomSheet isOpen={refundOpen} onClose={() => setRefundOpen(false)} title={t('refund.title')}>
